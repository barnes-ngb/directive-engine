#!/usr/bin/env npx tsx
/**
 * Run the pose-from-scan pipeline on the museum facade dataset.
 *
 * Usage: npx tsx src/pipelines/runMuseumPipeline.ts
 */
...
async function main() {
  console.log("╔════════════════════════════════════════════════════════════════╗");
  console.log("║  POSE FROM SCAN PIPELINE - Museum Facade                       ║");
  console.log("╚════════════════════════════════════════════════════════════════╝");

  const dataDir = "datasets/museum_facade_v0_1/directive_engine_export";
  const rawPath = path.join(dataDir, "museum_raw.json");
  const constraintsPath = path.join(dataDir, "museum_constraints.json");
  ...
  const config = {
    ...
    tube_radius_mm: 35,                // Segmentation radius
    synthetic_points_per_part: 100,    // Points per part
    synthetic_noise_mm: 3,             // Realistic noise level
  ...
  const outputDir = "datasets/museum_facade_v0_1/pipeline_output";
