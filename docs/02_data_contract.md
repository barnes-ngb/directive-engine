...
- `datasets/toy_facade_v1/expected_directives.json` — engine output for the
  fixture (regenerated by `npx tsx scripts/regenerate-toy-facade-expected.ts`)
- output: `directives.json` — directives + verification

`toy_facade_v1` ships nine panels in a 3 × 3 grid (Phase 6 expansion). One
panel is mildly deviated, one is the hero deviation that beat 2 dollies
in on, and one demonstrates the clamped failure mode. See
`datasets/toy_facade_v1/README.md` for the full role table.
